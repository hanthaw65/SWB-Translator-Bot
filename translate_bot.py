...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        "🌐 *SWB-Translator (Groq AI) မှ ကြိုဆိုပါတယ်!* 🌐\n\n"
        "ကျွန်တော်ကတော့ Groq AI ကို အသုံးပြုပြီး ဘာသာစကားများကို အခမဲ့ "
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["step"] == "selecting_source":
        if user_text in LANGUAGES:
            ...
            await update.message.reply_text("ကျေးပြု၍ အောက်က ခလုတ်ထဲမှ ဘာသာစကားတစ်ခုကို ရွေးပေးပါ။", reply_markup=get_lang_keyboard())
        return
    ...
    if state["step"] == "ready":
        ...
        translated_text = await translate_text_groq(user_text, state["source"], state["target"])
        await update.message.reply_text(translated_text)
    else:
        await start(update, context)

async def translate_text_groq(text: str, source: str, target: str) -> str:
    """Translate text using Groq AI API (Llama-3-70b)."""
    if not GROQ_API_KEY:
        return "❌ Error: Groq API Key မရှိသေးပါ။ Railway Variables ထဲမှာ GROQ_API_KEY ကို ထည့်ပေးပါ။"
...
if __name__ == '__main__':
    if not TELEGRAM_TOKEN or not GROQ_API_KEY:
        ...
        print("Bot is starting with Groq AI API...")
